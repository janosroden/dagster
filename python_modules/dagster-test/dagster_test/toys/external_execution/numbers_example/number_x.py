...
def main():
    if _get_storage_env() == "dbfs":
        from dagster_externals._io.databricks import DbfsContextSource, DbfsMessageSink

        source, sink = DbfsContextSource(), DbfsMessageSink()
    else:
        source, sink = ExternalExecutionFileContextSource(), ExternalExecutionFileMessageSink()

    context = init_dagster_externals(context_source=source, message_sink=sink)
